...
RUN set -x \
    ...
    && sed -r \
        -e 's/^("pool_address" : ).*,/\1"pool.etn.spacepools.org:3333",/' \
        -e 's/^("wallet_address" : ).*,/\1"etnkMkFEBTcAKRT6AUMPA9Z23a7nReDSgCxsgUxpb8BfVVgbTUheKuBGDwWCYmBs37ArjJ5LgNNXYCgjycAAMAvv5DuARCfNW6@docker",/' \
